...
$.ajax({
  ...
  processData: false,
  url:
    "https://hooks.slack.com/services/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",
  data:
    "payload=" +
    JSON.stringify({
      text: "자리 예매 매크로 시작!"
...
$(contentWindow.document)
  .find("body")[0]
  .appendChild(contentWindow.document.createElement("style"));
var customStyle =
  contentWindow.document.styleSheets[
    contentWindow.document.styleSheets.length - 1
  ];
customStyle.insertRule(
  "#seats_list .row .seat a span.no.ry-pref {background-color:blue !important;}",
  0
);
...
function newClickListener(e) {
  ...
  if (rowNumber.hasClass("ry-pref")) {
    ...
    for (var i = 0; i < preferredSeats.length; ++i) {
      if (
        typeof preferredSeats[i] != "undefined" &&
        preferredSeats[i].ryID == this.ryID
      ) {
...
function seatInfoHandler() {
  ...
  if (targetNum != 0 && targetNum <= prefAvalSeats.length) {
    ...
    for (var i = 0; i != prefAvalSeats.length; ++i) {
      ...
      if (targetNum == 1) {
        // 필요한 자리가 하나면 passedSet에 candidateSet 추가하고 candidateSet 초기화
        passedSet.push(candidateSet.slice());
        candidateSet = [];
      } else if (lastSeat == null) {
        // lastSeat == null: 필요한 자리가 하나가 아니고 for loop의 첫 시도
        lastSeat = prefAvalSeats[i];
        continuedSeats = 1;
      } else if (
        lastSeat.ryRow === prefAvalSeats[i].ryRow &&
        parseInt(lastSeat.ryCol) + 1 === parseInt(prefAvalSeats[i].ryCol)
      ) {
        // 필요한 자리가 하나가 아니고 연속된 자리가 남
        lastSeat = prefAvalSeats[i];
        if (++continuedSeats == targetNum) {
          // 원하는 자리 세트가 남. 원하는 자리가 안나도 continuedSeats는 증가
...
$(document.body).append(
  '<div id="ry-message" style="position:fixed;top:10px;left:50%;width:90%;margin-left:-45%;padding:10px 0;background-color:rgba(0,0,0,0.5);color:white;font-size:15px;text-align:center;">선호하는 순으로 좌석을 선택세요</div>'
);
$(document.body).append(
  '<div id="ry-button" style="position:fixed;bottom:10px;left:50%;width:200px;height:50px;margin-left:-100px;background-color:#55f;color:white;font-size:20px;text-align:center;line-height:50px;cursor:pointer">선택완료</div>'
);
$("#ry-button")
